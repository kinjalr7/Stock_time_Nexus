...
const Dashboard: React.FC = () => {
  ...
  const generateForecasts = () => {
    ...
    const lastHistoricalPoint = currentStockData.historicalData[currentStockData.historicalData.length - 1];
    ...
    for (let i = 1; i <= forecastHorizon; i++) {
      ...
      if (selectedModels.includes('Prophet')) {
        point.Prophet = lastPrice + Math.sin(i / (forecastHorizon / 4)) * (i * 0.2) + (Math.random() - 0.5) * 5;
      }
  ...
  const fullChartData = currentStockData ? [...currentStockData.historicalData, ...forecastData] : [];
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 pt-20 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Interactive Forecast Dashboard</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
          <div className="lg:col-span-3 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
             <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Forecasting Controls</h3>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Select Models</h3>
            <div className="space-y-3">
              {Object.entries(modelDetails).map(([name, { color, description }]) => (
                <div key={name} className="flex items-center">
                  ...
                  />
                  <label htmlFor={name} className="ml-3 text-sm text-gray-700 dark:text-gray-300">{name}</label>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
              {currentStockData ? `${currentStockData.name} (${currentStockData.symbol})` : 'Select a stock'}
            </h2>
          </div>
          <div className="h-96">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={fullChartData}>
                ...
                />
                <Legend />
                <Line type="monotone" dataKey="close" name="Historical" stroke="#8884d8" dot={false} strokeWidth={2} />
                {selectedModels.map(model => (
                  <Line key={model} type="monotone" dataKey={model} stroke={modelDetails[model as keyof typeof modelDetails]?.color} dot={false} strokeWidth={2} strokeDasharray="5 5" />
                ))}
              </ComposedChart>
            </ResponsiveContainer>
          </div>
